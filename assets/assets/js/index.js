...
submitBtn.addEventListener("click", () => {
    ...
    if (val.trim() !== "" && userName.checkValidity() && val2=="summit2021") {
        generatePDF(val);
        text = "Downloading.";
      } 
      if(val.trim() == "" && val2.trim() == "" && val2!=="summit2021"){
        text = "Enter your name and the correct Validation key";;
      }
      else if(val.trim() == ""){
        text = "Enter your name";;
      }
      else if(val2.trim() == "" && val2!=="summit2021"){
...
const generatePDF = async (name) => {
    ...
    const Montserrat  = await pdfDoc.embedFont(fontBytes);
     ...
     firstPage.drawText(name, {
       ...
       size: 40,
       font: Montserrat ,
       color: rgb(0.0, 0.0, 0.17),
     });
   
    // Serialize the PDFDocument to bytes (a Uint8Array)
    const pdfDataUri = await pdfDoc.saveAsBase64({ dataUri: true });
    saveAs(pdfDataUri,name + " Certificate")
  };